...
authTest.describe("auth", () => {
  authTest.use({
    viewport: { width: devices["iPhone 13 Pro"].viewport.width, height: 800 },
  });
...
const promoBannerTest = (type: "text" | "image") => {
  test.describe(`with promo banner (${type})`, () => {
    ...
    test.beforeEach(async ({ mockEnvs, mockAssetResponse }) => {
      await mockEnvs(
        type === "text"
          ? ENVS_MAP.navigationPromoBannerText
          : ENVS_MAP.navigationPromoBannerImage
      );
      await mockAssetResponse(
        "http://localhost:3000/image.svg",
        "./playwright/mocks/image_svg.svg"
      );
      await mockAssetResponse(
        "http://localhost:3000/image_s.jpg",
        "./playwright/mocks/image_s.jpg"
      );
      await mockAssetResponse(
        "http://localhost:3000/image_md.jpg",
